import type { TxVerification, ChainBalance } from "./types";
 ...
 * POST /wallet/transactions with transaction_hash filter.
 */
...
/**
 * Get token balances using Allium Realtime API.
 * POST /wallet/balances endpoint.
 */
export async function getAlliumBalances(
  ...
  try {
    ...
    for (const item of data.items ?? []) {
      const rawBalance =
        parseFloat(item.balance_raw) / Math.pow(10, item.decimals);
