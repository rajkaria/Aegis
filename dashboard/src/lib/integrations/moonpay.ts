...
import type { FundingOption, MoonPayTransaction, MoonPayCurrency, MoonPayAvailability } from "./types";
...
const CURRENCIES_CACHE_TTL = 60 * 60 * 1000;
...
export async function getMoonPayCurrencies(): Promise<MoonPayCurrency[]> {
  ...
  try {
    ...
    if (!response.ok) return currenciesCache?.data ?? [];
...
export async function checkMoonPayAvailability(ipAddress?: string): Promise<MoonPayAvailability> {
  ...
  try {
    ...
    if (!response.ok) return { isAllowed: true, isBuyAllowed: true, isSellAllowed: true, alpha2: "US" };
...
export function validateMoonPayWebhook(
  ...
  if (expectedSignature !== signatureHeader) return null;
...
export function getMoonPayConfig(): {
  ...
  const features: string[] = ["buy-url", "sell-url", "swap-url"];
  ...
  return { hasApiKey, hasSecretKey, hasWebhookKey, widgetMode: hasSecretKey ? "embedded" : "external", features };
}
